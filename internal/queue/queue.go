...
// for task data; a checksum over the loaded rows detects drift so stale state is
...
func Open(db config.DB) (*Queue, error) {
	...
	q := &Queue{
		...
		statePath: config.StatePath(db.Name), maxCancels: db.MaxCancels, leaseTTL: ttl,
	}
	if err := q.load(context.Background()); err != nil {
		return nil, err
	}
	// Persist the reconciled baseline so the drift notice fires once, not on
	// every read. This writes only the state file, never task data.
