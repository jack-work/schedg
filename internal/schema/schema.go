...
// and metadata; the default mirrors the figaro-qua figtodo `todo` template
// (see the figaro-dev skill) plus an optional priority column and dep table.
package schema
...
// Default returns the figtodo-shaped schema.
func Default() Schema {
	return Schema{
		Table:        "todo",
		PK:           "id",
		...
		DDL: []string{
			...
			// priority is added via AddColumnIfMissing (dolt lacks ADD COLUMN IF NOT EXISTS).
		...
		SavedQueries: []SavedQuery{
			{"schedg-open", "Open tasks by priority then age", "SELECT id, priority, created_at, title FROM todo WHERE done=0 ORDER BY priority DESC, created_at ASC, id ASC;"},
			{"schedg-deps", "Dependency edges (task depends on prerequisite)", "SELECT d.task_id, t.title AS task, d.depends_on_id, p.title AS prerequisite FROM todo_dep d JOIN todo t ON t.id=d.task_id JOIN todo p ON p.id=d.depends_on_id ORDER BY d.task_id;"},
			{"schedg-ready", "Open tasks with no unmet (open) prerequisite", "SELECT t.id, t.priority, t.title FROM todo t WHERE t.done=0 AND NOT EXISTS (SELECT 1 FROM todo_dep d JOIN todo p ON p.id=d.depends_on_id WHERE d.task_id=t.id AND p.done=0) ORDER BY t.priority DESC, t.created_at ASC;"},
			{"schedg-meta", "Queue metadata (repo back-reference, etc.)", "SELECT k, v FROM schedg_meta ORDER BY k;"},
		},
	}
}
